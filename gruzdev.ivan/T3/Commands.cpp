...
void process_commands(const std::vector<Polygon>& polygons) {
    ...
    while (std::cin >> cmd) {
        if (cmd == "AREA") {
            ...
            if (!read_subcommand(subcommand)) continue;
        ...
        else if (cmd == "MAX" || cmd == "MIN")
        {
            ...
            Polygon target;
            if (!read_polygon(target)) continue;
...
bool do_intersect(const Polygon& poly1, const Polygon& poly2) {
    ...
    for (size_t i = 0; i < pts1.size(); ++i) {
        ...
        const Point& q1 = pts1[(i + 1) % pts1.size()];

        for (size_t j = 0; j < pts2.size(); ++j) {
            const Point& p2 = pts2[j];
            const Point& q2 = pts2[(j + 1) % pts2.size()];
